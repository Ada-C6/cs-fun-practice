...
def test_stretch
  ary = [18, 7, 4, 24, 11]
  if stretch(ary) == [9, 9, 4, 3, 2, 2, 12, 12, 6, 5]
    return true
  else
    return false
  end
...
def stretch(int_array)
  ...
  int_array.each do |i|
    if i%2 == 0 #if even
      ...
      list << (1 + i/2) #Ruby integers rounds down
      list << (i/2)
    end
  end
  return list
...
def test_num_unique_both
  list1 = [5, 7, 7, 7, 8, 22, 22, 23, 31, 35, 35, 40, 40, 40, 41]
  list2 = [1, 2, 11, 17, 19, 20, 23, 24, 25, 26, 31, 34, 37, 40, 41]
  list3 = [0, 0, 0]
  if num_unique(list1) == 9 && num_unique(list2) == 15 && num_unique([]) == 0 && num_unique(list3) == 1 && num_unique_2(list1) == 9 && num_unique_2(list2) == 15 && num_unique_2([]) == 0 && num_unique_2(list3) == 1
    return true
  else
    return false
  end
...
def num_unique(int_array)
  num_hash = {}
  int_array.each do |i|
      num_hash[i] = "awesome"
  end
  puts num_hash
  return num_hash.length
end
...
def num_unique_2(int_array) #coded when I didn't read the instructions for problem 3
  counter = 0
  int_array.each do |i|
